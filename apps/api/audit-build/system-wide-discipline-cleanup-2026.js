...
exports.isAbsenceFinanciallyValid = isAbsenceFinanciallyValid;
exports.isUninformedAbsentDisciplineStillValid = isUninformedAbsentDisciplineStillValid;
exports.classifyDatedAbsenceDeduction = classifyDatedAbsenceDeduction;
exports.classifyUninformedAbsentDisciplineEvent = classifyUninformedAbsentDisciplineEvent;
exports.isLegacyAmountGroupUniform = isLegacyAmountGroupUniform;
exports.computeLegacyAggregateExcessAmount = computeLegacyAggregateExcessAmount;
exports.computeSubstantiveInvariantsTrue = computeSubstantiveInvariantsTrue;
exports.computeAllCleanupInvariantsTrue = computeAllCleanupInvariantsTrue;
...
function isTrueLateRow(row) {
    if (row.status === client_1.AttendanceStatus.LATE)
        return true;
    if (row.status === client_1.AttendanceStatus.HALF_DAY &&
        row.lateMinutes > 0 &&
 ...
 */
function isAbsenceFinanciallyValid(status) {
    return status === client_1.AttendanceStatus.ABSENT || status === client_1.AttendanceStatus.UNINFORMED_ABSENT;
}
/**
 ...
 */
function isUninformedAbsentDisciplineStillValid(status) {
    return status === client_1.AttendanceStatus.UNINFORMED_ABSENT;
}
/**
 ...
 */
function classifyDatedAbsenceDeduction(evidence) {
    if (evidence.currentStatus == null)
 ...
 */
function classifyUninformedAbsentDisciplineEvent(evidence) {
    if (evidence.currentStatus == null)
        return 'INSUFFICIENT_EVIDENCE';
 ...
 */
function isLegacyAmountGroupUniform(amounts) {
    return amounts.every((a) => Math.abs(a - amounts[0]) < 0.01);
}
function computeLegacyAggregateExcessAmount(amounts, excessCount) {
 ...
 * exception.
 */
function computeSubstantiveInvariantsTrue(invariants) {
 ...
 */
function computeAllCleanupInvariantsTrue(invariants, isApply) {
...
function classifyDescriptionTemplate(reason, description) {
    const desc = description ?? '';
    if (reason === client_1.DeductionType.LATE_ARRIVAL) {
        const m = desc.match(/^Late arrival deduction — monthly occurrence (\d+)$/);
        if (m)
            return { templateMatched: 'LATE_ARRIVAL_MONTHLY_OCCURRENCE', extractedOccurrence: Number(m[1]), extractedIncidentDate: null };
        return { templateMatched: 'UNRECOGNIZED', extractedOccurrence: null, extractedIncidentDate: null };
    }
    if (reason === client_1.DeductionType.UNINFORMED_ABSENCE) {
        const dated1 = desc.match(/^Uninformed absence deduction \(2 days\) — (\d{4}-\d{2}-\d{2})$/);
        if (dated1)
            return { templateMatched: 'UNINFORMED_ABSENT_DATED', extractedOccurrence: null, extractedIncidentDate: dated1[1] };
        const dated2 = desc.match(/^Absent without approved leave \(2 days stipend\) — (\d{4}-\d{2}-\d{2})$/);
        if (dated2)
            return { templateMatched: 'ABSENT_DATED', extractedOccurrence: null, extractedIncidentDate: dated2[1] };
        if (/^Uninformed absence deduction \(2 days\)$/.test(desc) ||
    ...
    if (reason === client_1.DeductionType.DISCIPLINARY_FINE) {
        const m = desc.match(/^Missing checkout deduction — monthly occurrence (\d+)$/);
        if (m)
            return { templateMatched: 'MISSING_CHECKOUT_MONTHLY_OCCURRENCE', extractedOccurrence: Number(m[1]), extractedIncidentDate: null };
        return { templateMatched: 'UNRECOGNIZED', extractedOccurrence: null, extractedIncidentDate: null };
    }
    if (reason === 'EXTRA_LEAVE_REJECTED') {
        const m = desc.match(/^Extra Full Leave rejected — 1-day deduction — (\d{4}-\d{2}-\d{2})$/);
        if (m)
            return { templateMatched: 'EXTRA_LEAVE_REJECTED_DATED', extractedOccurrence: null, extractedIncidentDate: m[1] };
        return { templateMatched: 'UNRECOGNIZED', extractedOccurrence: null, extractedIncidentDate: null };
    }
    if (reason === client_1.DeductionType.UNPAID_LEAVE) {
        return { templateMatched: 'UNPAID_LEAVE_AGGREGATE', extractedOccurrence: null, extractedIncidentDate: null };
    }
    return { templateMatched: 'UNRECOGNIZED', extractedOccurrence: null, extractedIncidentDate: null };
}
async function fetchEvidenceForEmployee(employeeId) {
...
async function buildEvidenceBundle(employeeId) {
    const { attendanceLogs, auditLogs, disciplineEvents, leaveRecords, letters } = await fetchEvidenceForEmployee(employeeId);
    const attendanceByDate = new Map(attendanceLogs.filter((l) => l.type === client_1.AttendanceLogType.REGULAR).map((l) => [dateLabelOf(l.date), l]));
    const auditByLogId = new Map();
    for (const a of auditLogs) {
        const arr = auditByLogId.get(a.entityId) ?? [];
        arr.push(a);
        auditByLogId.set(a.entityId, arr);
    }
    const trueAugustLateIncidentCount = attendanceLogs.filter((l) => l.type === client_1.AttendanceLogType.REGULAR && isTrueLateRow(l)).length;
...
function classifyDateForUA(dateLabel, bundle) {
    const log = bundle.attendanceByDate.get(dateLabel) ?? null;
    const disciplineEvent = bundle.disciplineEvents.find((e) => e.category === client_1.DisciplineCategory.UNINFORMED_ABSENT && dateLabelOf(e.incidentDate) === dateLabel);
    if (disciplineEvent)
        return 'PROVEN_UA';
    if (log) {
        if (log.status === client_1.AttendanceStatus.UNINFORMED_ABSENT)
            return 'PROVEN_UA';
        const auditsForLog = bundle.auditByLogId.get(log.id) ?? [];
        const everWasUA = auditsForLog.some((a) => {
            const changes = a.changes;
            return changes?.previous?.status === client_1.AttendanceStatus.UNINFORMED_ABSENT;
        });
        if (everWasUA)
            return 'PROVEN_UA';
...
async function collectEvidenceAndClassify() {
    ...
    const evidenceByEmployee = new Map();
    ...
    const dupKey = (r) => `${r.payrollEntryId}::${r.reason}::${r.description}`;
    const dupCounts = new Map();
    for (const r of allRows) {
        if (r.template === 'UNINFORMED_ABSENCE_LEGACY_UNDATED')
            continue;
        const key = dupKey(r);
    ...
    const SAFE_DUPLICATE_REASONS = new Set([client_1.DeductionType.LATE_ARRIVAL, client_1.DeductionType.UNINFORMED_ABSENCE]);
    const duplicateGroups = [...dupCounts.entries()].filter(([, rows]) => rows.length > 1);
    const safeExactTargets = [];
    const safeAggregateTargets = [];
    const manualReview = [];
    const protectedPayroll = [];
    ...
    for (const [, rows] of duplicateGroups) {
        ...
        for (const extra of extras) {
            ...
            if (pe.status !== client_1.PayrollStatus.PENDING) {
    ...
    for (const r of allRows) {
        if (r.template === 'UNINFORMED_ABSENCE_LEGACY_UNDATED')
            continue;
        if (rowsAlreadyClassifiedAsDuplicate.has(r.deductionId))
            continue; // duplicate status takes precedence, never double-classified
        ...
        if (r.template === 'LATE_ARRIVAL_MONTHLY_OCCURRENCE') {
            ...
            const ev = bundle.disciplineEvents.find((e) => e.category === client_1.DisciplineCategory.LATE && e.occurrence === occ);
            if (ev) {
    ...
    for (const [employeeId, rows] of legacyByEmployee) {
        ...
        const datedUAIncidentDatesForEmployee = new Set(allRows
            .filter((r) => r.employeeId === employeeId && (r.template === 'UNINFORMED_ABSENT_DATED' || r.template === 'ABSENT_DATED') && r.extractedIncidentDate)
            .map((r) => r.extractedIncidentDate));
        const candidateDates = new Set();
        for (const l of bundle.attendanceLogs) {
            if (l.type === client_1.AttendanceLogType.REGULAR)
                candidateDates.add(dateLabelOf(l.date));
        }
        for (const e of bundle.disciplineEvents) {
            if (e.category === client_1.DisciplineCategory.UNINFORMED_ABSENT)
                candidateDates.add(dateLabelOf(e.incidentDate));
        }
        const provenUADates = [];
        const ambiguousUADates = [];
        for (const dl of candidateDates) {
            ...
            if (datedUAIncidentDatesForEmployee.has(dl))
                continue; // consumed by its own dated deduction
            if (dl >= DATED_FORMAT_FIRST_CONFIRMED_DATE)
                continue; // post-cutoff backstop
    ...
    for (const [employeeId, bundle] of evidenceByEmployee) {
        for (const l of bundle.letters) {
            ...
            if (vars?.reversed || vars?.reversedDueToShortLeave)
                continue;
            ...
            if (occKey == null || vars?.incidentDate == null)
                continue; // no exact same-incident key -> never a proven duplicate
    ...
    const letterManualReviewGroups = new Map();
    for (const [employeeId, bundle] of evidenceByEmployee) {
        for (const l of bundle.letters) {
            const vars = l.variables;
            ...
            if (occKey !== 'NONE' && vars?.incidentDate != null)
                continue; // already handled above as a proven-or-singleton group
...
function deriveLateDisciplineEventTargets(safeExactTargets) {
    return safeExactTargets
        .filter((t) => t.pairedLateDisciplineEventId != null)
        .map((t) => ({ disciplineEventId: t.pairedLateDisciplineEventId, pairedDeductionId: t.deductionId, employeeId: t.employeeId }));
}
// ─── FINGERPRINT ───────────────────────────────────────────────────────
...
async function main() {
    ...
    const disciplineEventTargets = [
        ...
        ...deriveLateDisciplineEventTargets(evidence.safeExactTargets).map((t) => ({
            ...
            incidentDate: null,
            currentAttendanceStatus: null,
            pairedDeductionId: t.pairedDeductionId,
    ...
    const payrollEntryByIdForFingerprint = new Map(evidence.payrollEntries.map((pe) => [pe.id, { id: pe.id, status: pe.status, totalDeductions: pe.totalDeductions, netStipend: pe.netStipend }]));
    ...
    const deltaByPayrollEntry = new Map();
    ...
    const allExactTargetsPending = evidence.safeExactTargets.every((t) => evidence.payrollEntryById.get(t.payrollEntryId).status === client_1.PayrollStatus.PENDING);
    const allAggregateTargetsPending = evidence.safeAggregateTargets.every((t) => evidence.payrollEntryById.get(t.payrollEntryId).status === client_1.PayrollStatus.PENDING);
    const noProcessedTouched = ![...evidence.safeExactTargets, ...evidence.safeAggregateTargets].some((t) => evidence.payrollEntryById.get(t.payrollEntryId).status === client_1.PayrollStatus.PROCESSED);
    const noPaidTouched = ![...evidence.safeExactTargets, ...evidence.safeAggregateTargets].some((t) => evidence.payrollEntryById.get(t.payrollEntryId).status === client_1.PayrollStatus.PAID);
    ...
    // check.
    const freshLegacyRowsByEmployee = new Map();
    ...
    const legacySelectedAmountByEmployee = new Map();
    ...
    const legacySelectedPhysicalAmountEqualsProvenAggregateExcessAmountPerEmployee = [...legacySelectedAmountByEmployee.entries()].every(([employeeId, selectedAmount]) => {
        ...
        if (!isLegacyAmountGroupUniform(amounts))
            return false; // must never have been auto-targeted at all
    ...
    const legacySelectedPhysicalAmountEqualsProvenAggregateExcessAmountGlobally = (() => {
        ...
        for (const [employeeId, rows] of freshLegacyRowsByEmployee) {
            const sig = evidence.legacyEvidenceSignatures.find((s) => s.employeeId === employeeId);
            if (!sig || sig.excessCount <= 0)
                continue;
            const amounts = rows.map((r) => r.amount);
            if (!isLegacyAmountGroupUniform(amounts))
                continue; // non-uniform excess is never in totalSelected either
    ...
    const noLegacyAggregateTargetWhenAmountIdentityIsUnproven = [...freshLegacyRowsByEmployee.entries()].every(([employeeId, rows]) => {
        const amounts = rows.map((r) => r.amount);
        if (isLegacyAmountGroupUniform(amounts))
            return true; // uniform -> identity proven, auto-targeting allowed
    ...
    const dryRunContainsZeroWritePaths = APPLY
    ...
    const noCurrentUninformedAbsentIsTargetedAsStaleFinancialDeduction = staleUA.every((t) => t.currentAttendanceStatus !== client_1.AttendanceStatus.UNINFORMED_ABSENT);
    const noCurrentAbsentIsTargetedAsStaleFinancialDeduction = staleUA.every((t) => t.currentAttendanceStatus !== client_1.AttendanceStatus.ABSENT);
    const absenceFamilyFinancialValidityMatchesBusinessPolicy = staleUA.every((t) => !isAbsenceFinanciallyValid(t.currentAttendanceStatus ?? ''));
    const staleUATargetsAreIndependentlyRevalidatedFromRawAttendanceEvidence = staleUA.every((t) => classifyDatedAbsenceDeduction({ currentStatus: t.currentAttendanceStatus }) === 'EXACT_DATE_STALE_UA');
    const staleTargetDeductionIds = new Set(evidence.safeExactTargets.map((t) => t.deductionId));
    const datedAbsenceRows = evidence.allRows.filter((r) => (r.template === 'UNINFORMED_ABSENT_DATED' || r.template === 'ABSENT_DATED') && r.extractedIncidentDate);
    ...
    const noFinanciallyValidAbsenceFamilyRowIsInSafeExactCleanup = datedAbsenceRows.every((r) => {
        ...
        const log = bundle?.attendanceByDate.get(r.extractedIncidentDate);
        if (!log)
            return true; // insufficient evidence rows never enter safeExactTargets
        if (!isAbsenceFinanciallyValid(log.status))
            return true; // genuinely stale — fine if targeted
    ...
    const uninformedToAbsentDoesNotReverseFinancialDeduction = datedAbsenceRows.every((r) => {
        const bundle = evidence.evidenceByEmployee.get(r.employeeId);
        const log = bundle?.attendanceByDate.get(r.extractedIncidentDate);
        if (log?.status !== client_1.AttendanceStatus.ABSENT)
            return true; // not this transition
        return !staleTargetDeductionIds.has(r.deductionId);
    });
    ...
    const disciplineEventValidityIsIndependentFromFinancialValidity = disciplineEventTargets
        ...
        .every((t) => {
        const matchingRow = evidence.allRows.find((r) => r.employeeId === t.employeeId &&
            (r.template === 'UNINFORMED_ABSENT_DATED' || r.template === 'ABSENT_DATED') &&
            r.extractedIncidentDate === t.incidentDate);
        if (!matchingRow)
            return true; // no co-located deduction at all — trivially independent
    ...
    const invariants = {
        ...
        allInvalidOccurrence9TargetsAreActuallyOccurrence9LateFinancialRows: occ9.every((t) => t.reason === client_1.DeductionType.LATE_ARRIVAL && /occurrence 9$/.test(t.description ?? '')),
        allNeverReachedTargetsAreActuallyUnreachableLateRows: neverReached.every((t) => t.reason === client_1.DeductionType.LATE_ARRIVAL && /no longer true-late/.test(t.subcategory)),
    ...
    const applyResult = await prisma.$transaction(async (tx) => {
        ...
        for (const t of [...evidence.safeExactTargets, ...evidence.safeAggregateTargets]) {
            ...
            if (!deduction)
                throw new Error(`DRIFT DURING APPLY: deduction ${t.deductionId} no longer exists.`);
            const payrollEntry = await tx.payrollEntry.findUnique({ where: { id: t.payrollEntryId } });
            if (!payrollEntry)
                throw new Error(`DRIFT DURING APPLY: payrollEntry ${t.payrollEntryId} no longer exists.`);
            if (payrollEntry.status !== client_1.PayrollStatus.PENDING) {
        ...
        for (const dt of disciplineEventTargets) {
            ...
            if (!existing)
                throw new Error(`DRIFT DURING APPLY: disciplineEvent ${dt.disciplineEventId} no longer exists.`);
        ...
        for (const lt of evidence.letterVoidTargets) {
            ...
            if (!letter)
                throw new Error(`DRIFT DURING APPLY: letter ${lt.letterId} no longer exists.`);
        ...
        for (const adj of payrollAdjustments) {
            ...
            if (!after)
                throw new Error(`VERIFICATION FAILED: payrollEntry ${adj.payrollEntryId} missing after mutation.`);
            if (after.status !== client_1.PayrollStatus.PENDING) {
        ...
        const anyProtectedTouched = payrollAdjustments.some((adj) => {
            ...
            return pe.status !== client_1.PayrollStatus.PENDING;
        });
